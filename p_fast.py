...
for _ in range(trials):
    exp, torch_exp,tf_exp, _var_exp, var_torch, _var_tf, del_var, del_var_torch, del_var_tf = create_exp()

    var_exp = compile(_var_exp, '<string>', 'exec')
    var_torch = compile(var_torch, '<string>', 'exec')
    var_tf = compile(_var_tf, '<string>', 'exec')
    exp = compile(exp, '<string>', 'exec')
    torch_exp = compile(torch_exp, '<string>', 'exec')
    tf_exp = compile(tf_exp, '<string>', 'exec')
    ...
    with tf.GradientTape() as tape:
        ...
        vars = [eval(a.split(' =')[0]) for a in _var_tf.split(';')[:-1]]
        start = time()
        grads = tape.gradient(L_tf, vars)
        elapsed = time() - start
        print(f'TF: {elapsed}')
        tf_times.append(elapsed)

    '''vars = [a.split(' =')[0] for a in _var_exp.split(';')[:-1]]
    i = len(vars)
...
plt.plot(range(trials), times, label='Experiment')
plt.plot(range(trials), torch_times, label='PyTorch')
plt.plot(range(trials), tf_times, label='TF')
plt.legend()
